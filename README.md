...
- 以 `NODE_ENV=production` 启动，并写入管理员凭据哈希
...
如果服务启动失败，先检查 `data/users.json`、`data/messages.json`、`data/messages.jsonl` 和 `data/admin_audit.jsonl` 是否是合法内容。
...
- 主站页面不再提供可见后台入口
- 账号密码：由服务器环境变量 `ADMIN_ACCOUNTS`，或 `ADMIN_USERNAME` / `ADMIN_PASSWORD_HASH` 控制
- 部署脚本会把 `ADMIN_USERNAME` / `ADMIN_PASSWORD` 转成 `ADMIN_PASSWORD_HASH` 后写入服务环境文件
